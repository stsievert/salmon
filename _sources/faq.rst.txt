...
2. By specifying :class:`~salmon.triplets.manager.Sampling`'s ``query`` field.
   This allows showing users specific queries at specific times.  e.g., "for
   the first query the user sees, show them a query with [these objects]."
...
1. By setting the ``probs`` field in
   :class:`~salmon.triplets.manager.Sampling`. If the YAML specifies
   ``sampling.probs: {"ARR": 80, "Random": 20}``, the ``Random`` sampler will
   be used 20% of the time.
2. By setting the ``detail`` field in
   :class:`~salmon.triplets.manager.Sampling`. This will ensure that the query
   is generated by a specific sampler (or which sampler will receive the answer
   if the query is pre-specified in your ``init.yaml`` per
   ":ref:`specificquestion`").
