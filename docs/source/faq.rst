...
How do I ask about a fixed set of questions?
--------------------------------------------

By using :class:`~salmon.triplets.samplers.Validation`.  You can specify a list
of queries, or generating ``n_queries`` random queries. See :ref:`alg-config`
and the :ref:`valconfig` section.
...
* One question? Use :class:`~salmon.triplets.samplers.Validation`. There's an
  example in :ref:`valconfig`.
* More than one question? Specify ``sampling.details`` (which is documented in
  :class:`~salmon.triplets.manager.Sampling`).

``sampling.details`` allows customizing the following:

* For any query, the sampler the user receives
* For any query, the query that the user answers.

Any combination of these can be specified. An example is in :ref:`valdetail`,
and the documentation for :class:`~salmon.triplets.manager.Sampling` has
another example.
